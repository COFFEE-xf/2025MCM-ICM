...
# 目的是在一个数据集中找到各项之间的关联关系，而这种关系并没有在数据中直接体现出来。
# 以超市的销售数据为例，当存在很多商品时，可能的商品组合数量达到了令人望而却步的程度，这是提取关联规则的最大困难。
# 因此各种关联规则分析算法从不同方面入手减少可能的搜索空间大小以及减少扫描数据的次数。
# Apriori算法是最经典的挖掘频繁项集的算法，第一次实现在大数据集上的可行的关联规则提取，
# 其核心思想是通过连接产生候选项及其支持度，然后通过剪枝生成频繁项集。

# 最小支持度是衡量支持度的一个阈值，表示项目集在统计意义上的最低重要性
# 最小置信度是衡量置信度的一个阈值，表示关联规则的最低可靠性
# 强规则是同时满足最小支持度阈值和最小置信度阈值的规则

# 项集是项的集合。包含k kk个项的集合称为k kk项集，如集合{牛奶，麦片，糖}是一个三项集
# 项集出现的频率是所有包含项集的事务计数，又称为绝对支持度或支持度计数
# 如果项集I的相对支持度满足预定义的最小支持度阈值，则I是频繁项集。如果有k项，记为L k

# Apriori算法的主要思想是找出存在于事务数据集中最大的频繁项集，
